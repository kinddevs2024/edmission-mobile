...
import { getAdminTickets, type AdminTicket } from '@/services/adminApi'
import { getApiError } from '@/services/auth'
import type { AdminStackParamList } from '@/navigation/types'
import { fontSize, fontWeight, radii, space, useThemeColors } from '@/theme'

type Props = StackScreenProps<AdminStackParamList, 'AdminTickets'>

const LIMIT = 25

export function AdminTicketsScreen({ navigation }: Props) {
  const { t } = useTranslation(['admin', 'common'])
  const c = useThemeColors()
  const [list, setList] = useState<AdminTicket[]>([])
  ...
  const load = useCallback(async (p: number, reset: boolean) => {
    setError('')
    try {
      const res = await getAdminTickets({ page: p, limit: LIMIT })
      setTotal(res.total)
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: c.background }]} edges={['bottom']}>
      <Text style={[styles.h1, { color: c.text }]}>{t('admin:mobileTicketsTitle')}</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            onPress={() => navigation.navigate('AdminTicketDetail', { ticketId: item.id })}
          >
            <Text style={{ color: c.text, fontWeight: fontWeight.medium }} numberOfLines={2}>
              {item.subject}
            </Text>
            <Text style={{ color: c.textMuted, fontSize: fontSize.xs, marginTop: 4 }}>
              {item.userEmail ?? item.role} · {item.status}
            </Text>
          </Pressable>
        )}
      />
